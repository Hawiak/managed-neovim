...
local function build_lines(manifest)
	...
	local lines = {
		...
		"  Security",
		"  " .. string.rep("─", width - 2),
		string.format("  %-22s %s", "Sandbox", sandbox and "● running (macOS sandbox-exec)" or "○ not active"),
		string.format("  %-22s %s", "File locks", wrapper and "● enabled (chflags uchg)" or "○ not active"),
		string.format(
			"  %-22s %s",
			"Wrapper",
		...
		string.format("  %-22s %s", "Opened at", now),
		string.format("  %-22s %s", "Manifest path", vim.env.MANAGED_NVIM_MANIFEST_PATH or "not set"),
		"",
		string.rep("─", width),
		"",
		"  Audit Log (last 5)",
	...
	for _, l in ipairs(build_audit_lines()) do
...
local function set_highlights(buf)
	...
	vim.api.nvim_buf_call(buf, function()
		...
		match([[  Audit Log.*]], "ManagedNvimSection")
